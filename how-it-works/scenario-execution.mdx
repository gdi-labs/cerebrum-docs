...
6. On failure, format the `BuildResult`-shaped error into a feedback
   string and stash it under `build_feedback` so the next L4 retry picks
   it up. Register failure with the escalation tracker on conflict id
   `build_failure`.
