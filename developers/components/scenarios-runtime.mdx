...
* **Budget kill switch.** Scenario-wide caps (cost / tokens / duration) fire
  `budget_exceeded` and short-circuit subsequent levels with synthetic skipped
  states; the run ends with `scenario_finished status="aborted"`.
...
* `executor.py` — `ScenarioExecutor` dispatches each level to its kind-specific
  ...
  one release).
* `runners.py` — `PromptLevelRunner`, `VerifierLevelRunner`, `FanOutLevelRunner`,
  `HumanApprovalLevelRunner`. See
...
* `prompts.py` — Registered prompt builders for every scenario level
  ...
  prior-level outputs from `state["levels"]`.
