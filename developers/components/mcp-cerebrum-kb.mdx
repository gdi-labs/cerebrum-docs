...
description: A read-only MCP server that exposes the Cerebrum knowledge hub to any
  MCP client — Claude Desktop, Claude Code, partner tools, agents — through a single
  uniform contract. The same image serves three transports; choose at runtime.
---
